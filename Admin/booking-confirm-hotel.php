...
$favailable= $room_available['favailable'];
  ...
  $Check_out_date1=mysqli_query($conn, "SELECT * FROM hotel_enquiry WHERE hotel_id= '$_SESSION[hotel_id]' and room_type_id= '$_SESSION[room_type_id]' and deletedAt is NULL ");
  $check_out1= mysqli_fetch_assoc($Check_out_date1);
  $checkout1= $check_out1['checkout'];
  //$checkout= strtotime($checkout1);
  //$checkin= strtotime($enq_hotel_checkout);
  
  //$update_available_room1= $available+$addroom;

  // if($checkin==$check_out && $available<=0 && $favailable>=0)
  // {
  	// $update_available_room_query1= "UPDATE room_type SET available= '$update_available_room' WHERE hotel_id= '$_SESSION[hotel_id]' and room_type_id= '$_SESSION[room_type_id]' "; 	
  	// mysqli_query($conn, $update_available_room_query1);
  // }

  if($available>=0)
  {
	  if( $enq_hotel_checkin==$checkout1 )
	  {
